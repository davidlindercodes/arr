<?php $button = get_field("future_proof_button"); ?>
<div style="background-color: rgba(237,241,245);"> 
  <section class="relative mx-auto sm:text-center container mx-auto py-10" >
<!-- header -->
<h2 class="text-3xl sm:text-4xl font-bold mx-10 md:mx-0 text-center mb-10"> The Commitee </h2> 
<!-- description -->
...
<!-- Regular non-slider section for desktop -->
    <div class="hidden xl:block xl:flex xl:block flex-1 justify-center flex-row flex-wrap text-center">
      <?php while (have_rows("the_members")):the_row(); ?>
          <div class="relative inline-flex mx-0 px-4">
            <div class="service-card" >
              <img class="mx-auto mt-14 rounded-full services-image" src="<?php echo get_sub_field('photo')['url']; ?>" alt="<?php the_sub_field("name"); ?>"/>
              <h3 class="text-2xl 2xl:text-3xl text-blue tracking-wide mt-3 ">
                <?php the_sub_field("name"); ?> 
              </h3>
              <p class="mb-0 poacity-50" style="height: 47px;"><strong> <?php the_sub_field("position"); ?> </strong> </p>
              <p class="text-base 2xl:text-lg mb-10"> 
                <?php the_sub_field("fun_fact"); ?> 
              </p>
            </div>
          </div>
      <?php endwhile; ?> 
